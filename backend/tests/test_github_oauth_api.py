...
def _override_service(client, session):
    ...
    test_app.dependency_overrides[get_github_oauth_service] = _get_service
    return store


async def test_login_redirects_to_github(client, session):
    _override_service(client, session)
    response = await client.get("/api/v1/auth/github/login", follow_redirects=False)
    assert response.status_code == 302
    location = response.headers["location"]
    assert location.startswith("https://github.com/login/oauth/authorize")
    client._transport.app.dependency_overrides.pop(get_github_oauth_service, None)
...
async def test_callback_denied_returns_401(client, session):
    store = _override_service(client, session)
