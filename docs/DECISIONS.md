...
## ADR-087 — CD is a manually-triggered, approval-gated GitHub Actions workflow, not an automatic deploy on every push to main (2026-09-17)

**Context.** Phase 18 adds `.github/workflows/deploy.yml`: build images, push to ECR, run database migrations, `helm upgrade` the EKS workloads, verify the rollout. The obvious default for a CI/CD pipeline is "green build on main triggers a deploy." AgentABI's actual environment, decided in ADR-086, is the opposite of what that default assumes: there usually *is no running environment* to deploy onto — it is provisioned on demand for a demo and destroyed afterward. An automatic deploy-on-push would either silently no-op against infrastructure that doesn't exist (confusing failure, every single push) or — worse — implicitly encourage keeping the environment up continuously just so the pipeline has somewhere to deploy, undoing ADR-086's entire cost rationale. There is also no team of reviewers or an SRE rotation watching this pipeline; a bad deploy to a recruiter-facing demo has no one positioned to notice and roll it back within minutes the way a real on-call team would.

**Decision.** `deploy.yml` triggers only on `workflow_dispatch` (a human explicitly runs it, choosing an image tag and confirming the `demo` environment) and every job in it runs under a GitHub Environment (`demo`) configured with required reviewers, so even a manually-dispatched run pauses for an explicit approval before it authenticates to AWS. `ci.yml` (lint, typecheck, test, Terraform validate, security scan, Docker build-only) remains fully automatic on every push/PR — that is the quality gate, and it never touches AWS or pushes anything. The two are deliberately separate workflows so "does this code pass its own checks" and "should this be deployed to the shared demo environment right now" stay two different questions, answered at two different times, by a human for the second one.

**Consequences.** Turning AgentABI's demo environment on and updating it are always deliberate acts, matching the provision → demo → destroy lifecycle from ADR-086 — nobody is surprised by an automatic deploy firing against infrastructure that was deliberately torn down, and nobody accidentally leaves the environment up around the clock just to keep a pipeline green. The trade-off is that this is not continuous deployment in the usual sense: merging to `main` is necessary but not sufficient for a change to reach the demo environment, and someone has to remember to actually run the workflow (and be a reviewer on the `demo` GitHub Environment) before a recruiter-facing demo. That is an acceptable, even correct, trade-off for a single-maintainer portfolio project with an intentionally ephemeral environment — it would not be the right default for a team product with an always-on production environment and an on-call rotation.

## ADR-086 — AWS infrastructure is production-grade and reproducible, but the environment itself is ephemeral/on-demand, not always-on (2026-09-17)

**Context.** Phase 17 adds Terraform for a real AWS deployment target (EKS, RDS, ElastiCache, MSK, ECR, Secrets Manager, IAM/IRSA, optional Route53/ACM). AgentABI is a recruiter/portfolio project, not a production SaaS with paying users — nobody is harmed if the environment isn't reachable at 3am. Keeping this stack online continuously would cost real money every day (NAT gateways, the EKS control plane, RDS/ElastiCache/MSK instances, load balancers) for essentially zero demo-hours of actual use. The alternative of quietly shrinking the architecture into something cheaper-but-unrealistic (a single EC2 box, SQLite instead of RDS, no Kafka) would misrepresent the project's actual design and defeat the point of building it on EKS/RDS/MSK in the first place.

**Decision.** Two separate concerns are kept separate: the *architecture* (production-shaped: multi-AZ VPC, managed EKS with IRSA, managed Postgres/Redis/Kafka, private data plane, encrypted storage, least-privilege IAM) stays exactly as a real production deployment would look, with no toy substitutions. The *environment's uptime* is treated as a lifecycle, not a constant: `terraform apply` to provision → demo → `terraform destroy` (or scale-to-zero for the cheap-to-idle pieces) to stop cost accrual → `terraform apply` again to recreate on the next demo. This is made practical by: (1) every genuinely stateful thing that must survive a destroy/recreate cycle either lives outside the destroyable environment (Terraform remote state in a separately-bootstrapped S3 bucket) or has an explicit, documented opt-in preservation path (RDS final snapshot + `deletion_protection`); (2) nothing required to *understand* the project — the GitHub repo, architecture docs, ADRs, screenshots — depends on AWS being online; (3) cost-relevant knobs (NAT gateway count, MSK serverless-vs-provisioned, RDS Multi-AZ, disposable-vs-snapshotted database) are Terraform variables with cost-conscious defaults, not hidden constants, so the same codebase can be pointed at a cheap demo profile or a more production-like profile without editing module internals.

**Consequences.** A recruiter or interviewer can see the real architecture in code and docs at any time, and see it running live during an actual demo window, without AgentABI's owner paying for idle AWS capacity between demos. The trade-off is operational: every demo requires a `terraform apply` (tens of minutes for EKS to become ready) rather than the environment simply always being up, and anything not explicitly given a preservation path is lost on `terraform destroy` by design. `infra/terraform/README.md` documents the exact start/demo/shutdown/destroy/recreate procedure and, per resource category, whether stopping workloads actually stops the charges (it frequently does not — e.g., an idle EKS control plane and NAT gateways bill regardless of pod count) so the boundary between "scaled down" and "no longer costing money" is never left implicit.
